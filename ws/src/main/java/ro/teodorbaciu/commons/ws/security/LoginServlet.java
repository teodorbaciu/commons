...
/**
 * <p>
 * Handles the login process. This servlet can be used for handling the login process
 * initiated from an html form or for performing programatic login, so the session can be used
 * for calling webservice methods that require authentication.
 * </p>
 * <p>
 * We do a programatic login by specifying the <code>only-send-http-response</code> request parameter. The servlet
 * will only send back the corresponding http codes and will not try to redirect to the destination pages.
 * </p>
 * <p>
...
public abstract class LoginServlet extends HttpServlet {
	...
	/**
	 * Specified in web xml as a servlet parameter. The servlet will redirect to this relative path
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
				...
				req.getSession().setAttribute(
						WsConstants.KEY_CURRENT_USER, user);
			...
			log.error("Error checking auth info", e);
	...
	/**
	 * Used to correctly redirect to the specified path. We need this complex redirection
	 * when accessing the web app through a reverse proxy over ssl.
	 * @param req
	 * @param resp
	 * @param path
	 */
	private void redirectTo(HttpServletRequest req, HttpServletResponse resp, String path) 
	throws IOException {
	...
	/**
	 * Utility method to be overridden to perform additional processing
	 * after the login.
	...
	throws ServletException, IOException { return true;}
	
	/**
	 * Searches for the user with the specified
	 * username.
	 * @param username the username of the user :)
	 * @return an instance of {@link WtUser} or null if the 
	 * user was not found.
	 */
